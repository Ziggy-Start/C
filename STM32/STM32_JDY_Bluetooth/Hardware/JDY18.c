...
void JDY18_Init(void)     //初始化串口3
{
	  RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOB,ENABLE);
	  RCC_APB1PeriphClockCmd(RCC_APB1Periph_USART3,ENABLE);//两时钟顺序不能乱 ,串口3在总线1上
		...
		/*中断输出配置*/
		USART_ITConfig(USART3, USART_IT_RXNE, ENABLE);			//开启串口3接收数据的中断
...
{
	USART_SendData(USART3, Byte);		//将字节数据写入数据寄存器，写入后USART3自动生成时序波形
...
void Serial3_SendNumber(uint32_t Number, uint8_t Length)
{
	...
	{
		Serial3_SendByte(Number / Serial3_Pow(10, Length - i - 1) % 10 + '0');	//依次调用Serial3_SendByte发送每位数字
	}
}
...
{
	Serial3_SendByte(ch3);			//将printf的底层重定向到自己的发送字节函数
	return ch3;
}
...
void Serial3_Printf(char *format, ...)
{
	...
	va_end(arg);					//结束变量arg
	Serial3_SendString(String);		//串口3发送字符数组（字符串）
}
...
void USART3_IRQHandler(void)      
{
	...
	if (USART_GetITStatus(USART3, USART_IT_RXNE) == SET)	//判断是否是USART3的接收事件触发的中断
	{
		...
		/*当前状态为0，接收数据包包头*/        //正确格式为: @xxx!!   包头:@ ,包尾:!!  xxx为要发送的内容
		if (RxState == 0)
		{
